#!/usr/bin/env python
"""Download Binance global long/short account ratio history and save to CSV."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch Binance global long/short account ratio history")
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("data/external/binance/long_short_ratio"),
...
def fetch_long_short_ratio(
    ...
    while cursor > since_ms:
        ...
        response = session.get(
            BINANCE_FUTURES_BASE + LONG_SHORT_ENDPOINT,
            params=params,
            timeout=timeout,
        )
...
def normalize_dataframe(rows: List[Dict[str, str]]) -> pd.DataFrame:
    ...
    if df.empty:
        raise RuntimeError("No long/short ratio records fetched; nothing to save.")
